...
---
title: "Dynamic Time Warping — Core Ideas"
...
![**Figure 1.** Sakoe--Chiba band (left) and Itakura parallelogram (right). Blue = allowed region for the warping path; grey = forbidden. The dashed diagonal represents perfect synchrony between the two series.](plots/fig6_constraints.png){fig-align="center" width="85%"}
...
![**Figure 2.** Schematic of a single ECG beat showing the P wave, QRS complex (Q, R, S), and T wave.](plots/fig1_ecg_schematic.png){fig-align="center" width="80%"}
...
![**Figure 3.** Mean ECG waveform ± 1 SD for each class in the ECG200 training set (100 observations). Normal beats show a sharp R-wave spike; MI beats show a depressed, broader profile.](plots/fig2_mean_waveforms.png){fig-align="center" width="90%"}
...
![**Figure 4.** Six randomly selected individual ECG beats — three per class. Intra-class timing variability is visible even within the normal class.](plots/fig3_individual_beats.png){fig-align="center" width="90%"}
...
![**Figure 5.** Local cost matrix C(i,j) (left) and accumulated cost matrix D(i,j) with the optimal warping path overlaid in red (right). Computed using a Sakoe--Chiba band of width 15.](plots/fig4_cost_matrices.png){fig-align="center" width="95%"}
...
![**Figure 6.** DTW three-way alignment: Normal beat (blue, top) vs. MI beat (red, bottom). Connecting lines show matched time points. The right panel plots the warping path against the main diagonal.](plots/fig5_threeway_alignment.png){fig-align="center" width="95%"}
